...
public class Agent {
	...
	public Action shoot;
	Puck puck;
	private List<ControllablePlayer> friendlyPlayers;
	private List<Player> opposingPlayers;
	public Agent(Puck puck, List<ControllablePlayer> friendlyPlayers, List<Player> opposingPlayers)
	{
		this.puck = puck;
	...
	public Action Continuous_Pop_Agent()
	{
		...
		if(act.boundPassFromPos == false)
		{
			...
			if(rand.nextInt(20) != 1)
				...
				openConditions.addAll(act.bindPassFromPos(act.player1.getLocation(rand.nextInt(255))));
			return action;
		}
		...
		// -------------------------------------------------
		System.out.println("7: plan size = " + plan.size());
		action.act = Action.Act.NewGoal; // a fix so that if we do get here we do not lock the program.
		return action; // should never arrive here!!
	}
	...
	{	
		...
		{	
			...
			if(!act.act.equals(Action.Act.Shoot))// do not remove the shoot, it will be changed for the next goal
				plan.remove(act);
			//TEST
			...
			else if(cond.name.equals(Condition.Name.HasPuck))
			{
				...
				else
				{
					if(((CondHasPuck)cond).player.getId() < 3) // defender
						ChosenAction = new Action(Action.Act.Pass, null, ((CondHasPuck)cond).player, null, ((CondHasPuck)cond).player.getLocation(102));
					else // attacker 
